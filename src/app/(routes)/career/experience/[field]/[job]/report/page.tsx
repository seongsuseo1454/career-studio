...
export default function CareerReport() {
  ...
  // ✨✨✨✨ 여기에 `books` useMemo 로직이 들어갈 거야! ✨✨✨✨
  // 추천 도서 가져오기: 사용자 학년 정보가 computed에 직접 없으므로,
  // query parameter에서 학년(gradeLevel) 정보를 직접 가져와 매핑하거나,
  // 현재 성적(grade)을 기반으로 학년 난이도를 추정하여 도서를 가져옵니다.
  const books: Book[] = useMemo(() => {
    // 쿼리 파라미터에서 gradeLevel이 있으면 우선 사용
    const urlGradeLevel = urlSearchParams?.get('gradeLevel')?.toLowerCase();
    
    // gradeLevel 값이 elem, middle, high 중 하나면 그대로 사용
    let userGradeKey: GradeKey = (urlGradeLevel === 'elem' || urlGradeLevel === 'middle' || urlGradeLevel === 'high')
      ? urlGradeLevel as GradeKey
      : 'elem'; // 기본값

    // url에 gradeLevel이 없으면 성적(grade) 기준으로 대략 매핑
    if (!urlGradeLevel) {
      const gradeToLevel: Record<string, GradeKey> = {
        'S': 'high',
        'A': 'high',
        'B': 'middle',
        'C': 'middle',
      ...
      userGradeKey = gradeToLevel[grade ?? 'D'] || 'elem'; // grade가 undefined일 수 있으니 'D'로 기본 처리
    }
    
    const allBooks = getBooksByGrade(themeKey);

    // allBooks[userGradeKey]가 null 또는 undefined일 경우 대비해 빈 배열 반환
    return allBooks[userGradeKey] ?? [];
  }, [grade, themeKey, urlSearchParams]); 
  // ✨✨✨✨ `books` useMemo 로직 끝! ✨✨✨✨
      ...
      {/* 추천 도서 (학년별 6권 보장) */}
        <section className="mb-8 mt-8 books-wrap">
          <h2 className="text-3xl font-black text-slate-800 mb-6 flex items-center gap-3">
            ...
            <span>추천 도서 — 학년별 맞춤 (각 6권)</span> {/* 문구 변경 */}
          ...
          <div className="grid md:grid-cols-3 gap-4 books-list">
            {(['elem', 'middle', 'high'] as GradeKey[]).map((g) => {
                const booksForGrade = getBooksByGrade(themeKey); // 전체 학년 책 가져오기
                const currentGradeBooks = (Array.isArray(booksForGrade[g]) ? booksForGrade[g] : []).slice(0, 6); // 현재 학년 책 6권만 슬라이스
